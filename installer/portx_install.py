...
Downloads the latest FRP binary from the official GitHub Releases page
and installs it to ~/Downloads/portx/frp.
...
def resolve_download_url(
    ...
    e.g. frp_0.61.0_darwin_arm64.tar.gz
    """
    for asset in assets:
...
def validate_archive(archive_path: Path) -> None:
    """Checks that the downloaded file is a valid gzip tar archive."""
    if not tarfile.is_tarfile(archive_path):
        _error(
            "Downloaded file does not appear to be a valid tar archive.\n"
            f"  Path: {archive_path}\n"
            "  The download may have been interrupted or the file is corrupt."
        )
...
def extract_and_install(archive_path: Path) -> None:
    """
    Extracts the FRP archive and places the `frpc` binary at INSTALL_PATH.
    FRP archives contain a top-level directory like frp_0.61.0_darwin_arm64/
    with frpc (client) and frps (server) inside.  PortX uses frpc.
    """
    ...
    with tempfile.TemporaryDirectory(prefix="portx_frp_") as tmp_dir:
        ...
        # Locate the frpc binary anywhere in the extracted tree
